...
describe('c-hot-tjenesteleverandor-sa-transferred-list details navigation contract', () => {
    it('keeps the modal and routes Vis mer info to the separate Experience page', () => {
        const template = fs.readFileSync(
            path.join(COMPONENT_DIRECTORY, 'hot_tjenesteleverandorSaTransferredList.html'),
            'utf8'
        );
        const controller = fs.readFileSync(
            path.join(COMPONENT_DIRECTORY, 'hot_tjenesteleverandorSaTransferredList.js'),
            'utf8'
        );
